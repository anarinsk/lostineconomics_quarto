...
- 행의 합은 그렇지 않다. 왜그럴까? 
- Negative의 크기를 $N$, Positive의 크기를 $P$라고 하자. 이때 유병률은 $\frac{P}{P+N}$이다. 즉, $P(\neg H_0)$.
...
만일 $P(E|\neg H_0)$의 값이 고정되어 있다면, p값, 즉 $P(E|\neg H_0)$값이 낮을수록 $P(H_0|\text{E})$ 역시 작아질 것이다. 하지만 위에서 보듯이 영가설과 대립가설의 분포가 겹쳐 있는 상황에서 p값이 작다는 것 즉, 1종 오류를 줄인다는 것은 2종 오류를 늘이게 된다. 
