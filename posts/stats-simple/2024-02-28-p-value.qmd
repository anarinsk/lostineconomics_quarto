...
D&D라는 게임은 다각 20면을 지닌 주사위를 쓴다. 만일 "녹색 모자를 쓰고 주사위를 던질 때 20이 더 잘나온다"는 대립 가설을 세웠다고 하자. 이때 영가설은 "녹색 모자를 쓰는 것은 주사위의 결과에 영향을 미치지 않는다"가 될 것이다. 그리고 녹색 모자를 쓴 채 주사위를 던져서 20을 얻었다. 이때 p값은 어떻게 될까? 0.05이다. 

$$
P(\text{E}|H_0) = \dfrac{1}{20} = 0.05
$$

약간 에누리를 더해서 10% 유의 수준에서 해당 영가설을 기각했다! 따라서 "녹색 모자를 쓰고 주사위를 던질 때 20이 더 잘 나온다!" 이 말은 맞나? 당연히 이상한 말이다. 수많은 D&D 플레이어들은 이 말이 틀리다는 사실을 경험적으로 너무나(!) 잘 알고 있을 것이다. 
...
| -------- | -------- | -------- |
| reject         | false positive ($\alpha$)  | true positive ($1-\beta$) |
| fail to reject | true negative ($1-\alpha$) | false negative($\beta$)   |

: Error types {#tbl-error}

|  | Negative | Positive |
| -------- | -------- | -------- |
| predicted Positive | false positive ($\alpha$)  | true positive ($1-\beta$) |
...
- false positive의 허용 확률 $\alpha$가 1종 오류(type I error)이다.  
- false negative의 허용 확률 $\beta$가 2종 오류(type II error)이다. 
...
증거 E를 관찰했을 때 영가설을 기각할 확률을 높이려면 위의 확률을 낮춰야 한다. $\alpha$와 $\beta$가 모두 낮을수록 해당 확률은 낮아진다. 그렇다면 이 둘을 모두 낮추면 좋지 않을까? 
...
Positive라고 보고한 결과, 즉 $H_0$을 reject한 연구 중에서 오직 40%($\frac{60}{60+150}$)만 타당한 결과다. 나머지 60%는 false positive에 속한다. 
