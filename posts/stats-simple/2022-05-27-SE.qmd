...
"유한 표본 finite sample"이란 어떤 크기든 표본의 크기가 정해져 있다는 의미이다. 가끔 "소 표본 small sample"이라는 표현도 활용된다. "점근적 특성 asymptotic properties"이란 표본 크기가 계속 커질 때 추정량이 지니는 특성을 의미한다. 통계학을 가르칠 때는 유한 표본의 추정량(estimator)에 관해 엄밀하게 내용을 배우지만, 실용적인 관점에서 중요한 것은 추량량의 점근적 특징이다. 

점근성에서 가장 중요한 내용은 두 개다. 일치성은 샘플의 크기가 커질수록 추정량이 모수에 접근하는 특징이다. 다른 하나는 중심 극한 정리이다. 샘플의 크기가 커질수록 추정량의 분포가 정규 분포에 접근하게 된다. 일치성이 확보되면 추정량이 불편성을 지니고 있지 않아도 표본의 크기가 커질수록 참 값이 충분히 가깝게 접근한다. 한편 중심 극한 정리는 추정량의 통계적인 검정을 위한 필수적인 방법을 제공한다. 
...
::: {layout="[[-10, 89, -10]]" layout-valign="bottom"}
...
$S^2_n$은 [불편 추정량](#s2-n-1)이고 [일치 추정량](#s2-consistent)이다. $n$이 충분히 크다면 불편 추정량 여부는 중요하지 않다. $\dfrac{1}{10000}$과 $\dfrac{1}{9999}$의 차이가 큰 의미가 있을까? $(n-1)$ 대신 $n$이 들어간 표본 추정량 역시 일치성을 지니고 있다. 
...
우선 $t-$통계량의 분자와 분모를 살펴보면, 각각 $n$이 커질 때 분자와 분모는 모두 0으로 수렴한다. 이 비율이 정규분포에 수렴하는 게 되는 이유는 중심극한 정리를 이용해 증명할 수 있다. 일단 결론만 기억하도록 하자. $t$-통계량은 표본 크기 $n$이 증가하면서 표준 정규 분포에 근사하게 된다. 즉 점근적으로 (표준) 정규분포(asymptotically normal)가 된다. 
...
![$n$이 커질수록 $\ES{X}_n$의 분포는 표준 정규 분포에 근접한다.](https://miro.medium.com/max/1400/0*V1zCLq5mboTCfq2_.jpeg)
:::
...
울드리지 교수는 이를 통계적 유의성과 실용적 유의성의 차이로 설명했다. 대체로 통계적으로 유의미한 변수를 찾으면 우리는 흥분하기 마련이다. 하지만 그 통계적으로 유의한 관계가 실용적으로도 그럴까? 만일 통계적 유의성을 지닌 계수가 사실상 의미가 없는 경우라면 어떨까? 책을 읽을 때 다리를 떨면 책을 읽는 속도가 1에서 1.01이 된다는 결과를 얻었다고 하자. 이는 유의미한 결과일까?  
