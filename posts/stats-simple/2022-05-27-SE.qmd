...
울드리지 교수는 이를 통계적 유의성과 실용적 유의성의 차이로 설명했다. 대체로 통계적으로 유의미한 변수를 찾으면 우리는 흥분하기 마련이다. 하지만 그 통계적으로 유의한 관계가 실용적으로도 그럴까? 만일 통계적 유의성을 지닌 계수가 사실상 의미가 없는 경우라면 어떨까? 책을 읽을 때 다리를 떨면 책을 읽는 속도가 1.01배 빨라진다는 결과를 얻었다고 하자. 이는 유의미한 결과일까?  
