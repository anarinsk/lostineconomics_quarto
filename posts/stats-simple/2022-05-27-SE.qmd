...
- 기본적인 통계학 용어와 개념을 정리해 보자. 
- 기본을 자주 잊어먹는 나를 위한 개인적인 용도

## Why

통계학을 들은 사람들이라면 표준 오차는 대체로 잘 아는 내용이다. 그런데 가끔 까먹는다. 개인적인 용도를 위해서 정리해둔다. 
...
:::{ .column-margin }
표본 $x_i$가 있다고 할 때, $\hat \theta$는 $x_i$를 정보를 통해 구성된다. 즉, $\hat\theta \equiv \hat\theta(x_i)$.
:::
...
|표본 분산|sample variance|표본 통계량의 분산|$S(Y_i)^2 = \frac{1}{n-1}\sum_{i=1}^n (Y_i - \ES{Y}_n)^2$|
...
|t-통계량|t-statistic|$t(\mu) = \frac{\overline{Y}-\mu}{\widehat{SE}(\overline{Y})}$|$n$이 커질 때 정규분포에 근접한다.|
...
2에 따라서 $n$이 증가함에 따라서 $\V(\cdot)$가 임의의 작은 수가 되기 때문에 $\ES{Y}_n \overset{P}{\longrightarrow} \mu$

:::{ .column-margin }
$\overset{P}{\longrightarrow}$는 확률적인 수렴, 즉 probability limit을 뜻한다. 
:::
...
![$n$이 커지면서 정규분포에 근접한다.](https://ars.els-cdn.com/content/image/3-s2.0-B0123693985001729-gr2.gif)

다시 말하면, $\ES{Y}_n$은 평균이 $\mu$이고 표준편차가 $\frac{\sigma}{\sqrt{n}}$인 정규 분포에 근사하게 된다. 따라서 $n$이 계속 커지면 결국 $\mu$로 집중된다. t-통계량은 대 표본(large sample)의 통계적 검정에 핵심을 이룬다. 원래 자료가 어느 분포에서 나왔는지와 관계없이 표본 통계량은 정규 분포를 따르게 되고, 이를 통해 가설 검정을 수행할 수 있다. 

:::{ .column-margin }
회귀 분석의 검정 또한 t-통계량에 근거한다. [여기](#regression-t-statistic)를 참고하도록 하자. 
:::

t-통계량을 활용할 때 각별하게 주의해야 한다. 이 값의 크기는 검정을 위한 통계학적 증거일 뿐이다. t-통계량의 형태에서 보듯이 분자가 충분히 크거나 혹은 분모가 충분히 작을 때 이 값이 커진다. 이는 신뢰 구간에 대해서도 마찬가지다. 신뢰 구간은 표준 오차에 반영되어 있는 통계적 정밀성에 의해 결정되지 우리가 발견하고자 하는 관계의 강도에 의해서 결정되지 않는다. 즉, t-통계량 및 신뢰 구간의 크기를 보고 해당 독립 변수가 종속 변수에 미치는 효과의 강도로 오해하면 곤란하다.  
...
- $x^r_i$: $X$의 $i$'th로 벡터 
- $\underset{(1 \times k)}{x^r_i}\underset{(k \times 1)}{\hat\beta} = \hat y_i$

회귀 분석의 $i$'th 계수인 $\beta_i$는 $t_i$를 통해 검정할 수 있다. $n$이 충분히 클 경우 $t_i \sim \rm N(0,1)$이 된다. 