...
앞서 정의한 콘셉트에서 $\vec v$의 크기는 점의 위치가 아니라 해당 방향으로 향하는 크기를 나타낸다는 점에 각별하게 주의하라. 
...
앞서 파리메트릭 표현에서 임의 $P$의 좌표 $(x, y, z)$는 다음과 같다. 
...
$$
P: \lbrace Ax + By + Cz = D \rbrace
$$

- Parametric equation

$$
P : \lbrace p_0 + s \vec v + t \vec w~\text{where}~s,t \in \mathbb R \rbrace
$$

- Geometric equation

$$
P : \lbrace (x, y, z) \in \mathbb R^3 | \vec n \cdot [(x,y,z) - p_0] = 0 \rbrace \text{~with normal vector $\vec n$}
$$


### Normal vector 

![](http://mathonline.wdfiles.com/local--files/point-normal-form-of-a-plane/Screen%20Shot%202014-06-14%20at%207.02.06%20AM.png){ style="margin: auto; display: block; border:1.5px solid #021a40; width: 40%;" }
