...
### Cardinality
집합이 '크다'는 것의 의미를 따져보자. 유한 집합의 경우 크다는 뜻이 자명하다. 무한 집합의 크기는 어떻게 비교할 수 있을까? 무한 집합에 대해서는 원소의 '개수'라는 개념을 쓸 수 없다. 대신 크기의 비교를 위해서 카디널 수(cardinality)라는 말을 쓴다. 무한 집합 $A$에 대해서 $|A|$라고 쓸 경우 이는 무한 집합의 크기, 즉 카디널 수를 의미한다. 

자세한 내용을 생략하고 말한다면, 카디널 수는 자연수 집합을 기준으로 집합의 크기를 가늠한다. 자연수($\mathbb N$)는 가장 작은 무한 집합이다. 만일 자연수 집합과 일대일 대응이 가능하다면 둘의 카디널 수는 같다. 즉 둘은 같은 크기의 집합이 된다. 

### Injection, Surjection, and Bijection 

집합의 크기를 재려면 집합과 집합 사이에 어떤 짝짓기(mapping)이 가능한지를 살펴야 한다. 용어 및 번역어 먼저 정리하자. 
...
이제 칸토어의 정리를 살펴보자. 정리의 대단한 내용과 비교해서 증명은 매우 간단하다. 임의의 집합 $A$에 대해서 멱집합 $\mathcal P (A)$는 $A$보다 크다. 즉, 
