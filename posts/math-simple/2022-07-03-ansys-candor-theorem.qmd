...
:::{.column-margin}
[여기](https://plato.stanford.edu/entries/russell-paradox/)를 참고하자. 소박한 집합론(naive set theory)에 따르면 특정한 특성을 바탕으로 집합을 정리할 수 있다. 하지만 이러한 공리에 구멍이 있음을 러셀이 발견했다. 즉, $\phi(x)$라는 특성은 $x \in x$라고 하자. $\pi(x)$는 집합 자체가 그 자신의 원소가 되는 특성까지 포함한다. 이제 $R = \{ x: \neg \phi(x) \}$라고 하자. 이제 $R \in R$을 만족한다고 하자. $R$의 정의상 $R \notin R$이어야 한다. 따라서 모순이다. 만일 $R \notin R$이라고 하자. $R$의 정의상 $R \in R$이어야 한다. 따라서 모순이다. 
:::