...
진리 집합으로 다시 따져보도록 하자. 진리 집합은 조건제시법(set builder notation)을 통해 표현한다. 일단 한정사를 제외하고 $P$와 $Q$를 표현해보자. 
...
진리 집합을 정의할 때 다소 까다로운 것은 한정사이다. 한정사의 경우 명제를 부정할 경우 전체가 바뀌게 된다. 따라서 $P$의 진리 집합에 적용되는 한정사는 $\forall \delta > 0$가 된다. $Q^C$에 적용되는 한정 명제는 $\exists \epsilon > 0$가 된다. 이 점을 고려하면, 
$$
\begin{aligned}
P & = \{ x|~\forall\delta>0: |x - a| < \delta \} \\
