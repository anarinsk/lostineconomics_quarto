...
$\hat\epsilon_i = Y_i - \hat{Y}_i$가 정규 분포를 따르면 될까? 이 역시 마찬가지로 $\epsilon_i$가 정규 분포를 따르리라는 보장이 없다. $\hat\epsilon_i$는 회귀 분석의 모델링 아래 도출된 $\epsilon_i$에 관한 추정치일 뿐이다. 추정치가 정규 분포를 따른다고 해서 원래 대상이 꼭 그렇다고 볼 수 있을까? 
...
- $\hat y_i = \underset{(1 \times k)}{x^r_i}\underset{(k \times 1)}{\vphantom{x^r_i}\hat\beta}$,  $x^r_i$: $X$의 $i$'th 행(row) 벡터 
