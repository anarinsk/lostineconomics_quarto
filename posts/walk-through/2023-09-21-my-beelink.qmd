...
"웹 서버"라고 제목에 쓰긴 했지만 사실 단순한 파이썬 앱 호스팅, 토렌트 다운로드, 그리고 DLNA 서버 정도 만드는 것이 전부이다. 즉 본격적인 의미의 NAS를 만드는 것이 아니다. 깃헙과 스트리밍 서비스가 존재하는 시대에 굳이 개인용 NAS까지 만들 정도로 보관해야 할 자료가 없기도 하다. 

## 서버 구매 

최대한 싸고 간단하게 구축하는 것이 목표이다. 처음에는 라즈베리 파이를 고민했으나 이런 저런 부품을 구비하려면 그 비용이 만만치 않더라. 알리익스를 검색해보니 N95와 같은 준수한 저전력 x86 CPU를 장착한 베이본 물건을 13만원 정도면 살 수 있었다. 윈도OS도 필요 없고 메모리와 nvme SSD는 이미 굴러다니는 게 있어서 이 녀석을 구매했다. 16GB 메모리는 약 5.5만원 정도, 512GB SSD는 4만원 정도면 구매하니, 대략 10만원 정도 더하면 쓸만한 작은 서버를 살 수 있는 셈이다. 
...
- 설치가 끝는 이후에는 별도의 기기를 통해 ssh 접속하는 게 작업하기에 편리하다. 서버에 디스플레이와 키보드를 붙여서 쓴다면 명령어를 일일이 타이핑 해야 한다. 

## 설치 후 작업 

우분투 설치 후에는 몇가지 작업을 해주자. 
...
이 소박한 가정용 서버의 목적이 streamlit을 호스팅하기 위한 것이다. 외부 클라우드 컴퓨팅을 사용하면 허름한 것을 쓰더라도 월 만원 정도 나간다. 스트림릿에서 제공하는 무료 티어를 쓸 수도 있지만, 이 녀석은 자주 쓰지 않는 경우 파킹에 들어가기 때문에 사용성이 아주 좋지는 않다. 우분투 깔아서 직접 호스팅하는 편이 가정용 용도라도 꽤 편리하다. 서버 가격이 비싸지 않으면서, 저전력이라서 전기세도 부담스럽지 않다. 

#### 설치와 서비스

- pixi를 통해서 streamlit을 설치한다. 
- 프로세스를 계속 띄우기 위해서는 tmux를 활용하자. 
  - tmux세션을 만든 후에 
  - 해당 세션에서 streamlit 프로세스를 띄우고 
  - 디태치해서 빠져 나온다. 

```bash
> tmux new-session -s {내-세션} # 새로운 세션 시작 
...
```
- 호스팅 주소가 있다면 해당 서비스의 네임서버에서 `A` 방식으로 IP 주소를 적어주면 된다. 회사에 따라서는 열어 놓은 포트를 제한하는 곳이 있다. 위에서 스트림릿의 `server.port`는 이를 고려하기 위한 것이다. 

:::{.column-margin}
Cloudflare를 예로 들어보자. http와 https 별로 특정한 포트가 열려 있다. 해당 포트로만 접근이 가능하다. IP 주소를 감추기 위해 다음과 같은 우회로를 쓸 수 있다. A 호스트네임을 IP 주소와 연동한다. 필요한 서비스에 접속하기 위해서는 `A:8880`이런 식으로 접근하면 된다. 만일 8880도 없애고 싶다면 어떻게 해야 할까? CNAME으로 B 호스트네임을 자체 도메인으로 연동한다. 그리고 Rule의 리디렉션 룰을 생성해서 B 호스트네임을 `A:8880`으로 리디렉션하면 된다.  
:::
...
- 이때 넣고 싶은 폴더는 `anari` 그룹 안에 속해 있고 이용자는 `debian-transmission`이다. 
- 이 경우 `anari` 그룹 안에 `debian-transmission`이라는 이용자를 추가해주면 된다. 
