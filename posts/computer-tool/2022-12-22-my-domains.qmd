---
title: 보유 도메인을 GitHub Pages에 연결하자. 
description: 도메인을 도메인 답게!
author: "JS HUHH"
date: "12/22/2022"
image: "images/dns.jfif"
...
GitHub Pages는 훌륭한 스태틱 웹 서비스다. js, css 정도를 쓰는 html 문서를 전달하는 데에는 이 보다 싸고 편ㅗ한 서비스를 찾기 어려울 것이다. GitHub Pages에는 외부 도메인을 연결할 수 있다. 다만 연결을 위해 몇 가지 절차를 거쳐야 한다. 
...
자세한 내용은 [여기](https://docs.github.com/ko/pages/configuring-a-custom-domain-for-your-github-pages-site/verifying-your-custom-domain-for-github-pages)를 확인하자. 
...
### 도메인 연결 2 (GitHub Pages)

이제 연결을 윈하는 리포지터리의 `Settings` > `Pages`로 가자. "Custom domain" 메뉴에 앞서 설정한 주소, 즉 `www.lostineconomics.com`을 입력하고 저장하면 된다. 
...
빌어먹을 구글이 Domains 서비스를 접었다. 하지만 우리에게는 그에 못지 않은 훌륭한 대안이 있다. 도메인 구매와 DNS 관리 그리고 analytics까지 모두 제공하는 [Cloudflare](https://www.cloudflare.com/products/registrar/)를 추천한다. 

가입 후 웹 사이트를 등록하면서 DNS을 바꿔주면 그 이후 세부 설정을 할 수 있다. 이 홈페이지의 예시를 간략하게 스샷으로 소개하겠다. 도메인 관리의 모든 기능이 제공되면서도 인터페이스도 편리하다. 
