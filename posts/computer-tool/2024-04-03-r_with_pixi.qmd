...
![두 개 모두 적절한 위치로 바꿔 사용하자.](./images/pixi-r-2-min.png)
...
systemfonts 패키지가 설치되지 않는다. 이게 설치되지 않으면 languageserver, devtools 등등 여러 패키지가 설치되지 않는다. 발생하는 에러 메시지를 잘 보면서 OS 별로 대응하면 된다. 

하지만 때로는 시키는대로 해도 해결되지 않는 경우가 있다. 그럴 땐 어떻게 할까? R에서 이 문제는 C++ 컴파일러와 관련된 이슈다. 가장 깔끔하게 해결하려면 pixi shell로 접근해서 해결하면 된다. 즉, 
...
이렇게 설치하면 OS 상관 없이 대체로 잘 설치가 된다. 패키지를 업데이트하지 않는 이상 이상 설치된 이후 활용에는 별다른 문제가 없다. 
