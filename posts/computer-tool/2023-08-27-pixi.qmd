...
:::{.column-margin}
개발팀은 Rust의 `cargo` 명령어를 지향한다고 하며, 언어를 가리지 않지만 의존성 지옥에 가장 취약할 python에서 누릴 수 있는 이득이 가장 클 것이라고 밝히고 있다. 혹시나 해서 julia를 태워 봤는데 지원되지 않았는데, conda-forge를 통해서 지원되는 julia는 intel macos와 linux 뿐이었다. WSL에서는 잘 설치된다. 현재로서 랭귀지 지원은 conda 채널에 종속되어 있다.  
:::
