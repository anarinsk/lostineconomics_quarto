...
단점이 아예 없는 것은 아니다. 서비스가 anaconda 패키지 관리 시스템에 의존하기 때문에 필요한 패키지가 사용 환경에서 아나콘다 생태계에 없다면 어쩔 수 없이 타협해야 한다.
...
pixi는 conda를 기반으로 하기 때문에 PyPI 패키지를 설치할 수 없다. 대체로 문제가 되지 않지만, `matplotlib`, `stargazer` 등은 PyPI로만 설치할 수 있다. 다행히 0.8 버전부터 PyPI 패키지를 설치할 수 있게 되었다. 
