...
파이썬은 많은 의존성 관리 도구를 지니고 있지만 사실 좀 골치가 아프다. 일단 OS를 탄다. 그리고 의존성 관리에 필수적인 가상 환경도 주로 쓰는 하나가 아니다. 이렇다보니 조합이 이래저래 복잡해진다. 본격적으로 쓰려면 [Poetry](https://python-poetry.org/) 같은 녀석을 부려야 하겠지만, 적당히 문과적 수준에서 타협할 녀석이 있었으면 했다. Rye와 pixi가 딱 그 정도에 적합하다.  
