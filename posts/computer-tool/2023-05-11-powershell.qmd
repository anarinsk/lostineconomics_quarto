...
여기서 `false`가 뜬다면 프로필을 로드하는 경로에 문제가 생긴 것이다. `conda init powershell`을 다시 실행해보자. 만일 이떄에도 문제가 생긴다면 기본으로 잡힌 도큐먼트 폴더를 powershell에서 읽어들이지 못하는 문제에 해당한다. 이 문제 해결은 옆의 link 안의 내용을 참고하자. 
