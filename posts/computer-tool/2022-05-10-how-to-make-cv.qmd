...
## PDF라면 $\rm \LaTeX$!
정형화되고 멋진 PDF 문서를 만들어 내는 데에는 $\rm \LaTeX$만한 것이 없다. 아카데미에서 주로 사용하는 도구로서 논문 이력 관리를  bib$\rm TeX$으로 하고 있다면 제법 편리하고 멋진 이력서를 얻을 수 있다.
...
- `@import` | 한글 폰트를 로딩 하는 과정이다. 기본 폰트 이외에 특화된 한글 글꼴을 쓰기 위해서는 별도의 로딩이 필요하다. 여기서는 제목으로는 '나눔스퀘어'를 본문으로는 '프리텐다드'를 사용했다. 로딩은 CDN을 활용하는데, 약간의 시간이 필요하지만 활용에 지장을 줄 정도는 아니다. 
