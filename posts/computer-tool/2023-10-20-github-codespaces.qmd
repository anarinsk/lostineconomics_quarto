...
깃헙이 이미 이런 기능을 제공하고 있다. 깃헙 코드스페이시스(깃헙 코스)는 웹에서 개발과 컴퓨팅 환경을 상당한 수준으로 제공하는 서비스다. 노트북 환경에 제한된 구글 코랩과 달리 깃헙 코스는 온전한 우분투의 컴퓨팅 환경을 제공한다. 클라우드에 별도의 환경을 세팅하지 않는 이상 깃헙 코스를 능가하는 유연하고 편리한 컴퓨팅 환경을 얻기는 쉽지 않을 것이다.  
...
![](https://docs.github.com/assets/cb-50007/mw-1440/images/help/codespaces/who-will-pay.webp){width=50%}

그림처럼 깃헙 페이지의 오른쪽 상단에 `Code` 버튼이 생기는데, 이 버튼을 누르면 `Open with Codespaces` 메뉴가 나타난다. 이 메뉴를 선택하면 깃헙 코스가 활성화된다.

:::{.column-margin}
깃헙 코스의 장점은 리포의 브랜치에 따라서 서로 다른 코스를 붙일 수 있다는 것이다. 비슷한 개발 환경을 미묘하게 바꿔야 할 필요가 있다면 유용한 기능이 될 것이다. 
:::

활성화된 이후에는 VS Code와 동일한 환경이 제공된다. 별도로 구구절절하게 설명할 것이 없다는 이야기다. 자세한 것은 아래 페이지를 참고하면 된다. 

<https://docs.github.com/ko/codespaces/overview>

## 나의 활용법 

깃헙코스를 내가 어떻게 쓰고 있는지 간단히 소개하겠다. 물론 소개는 미래에 모든 것을 망각할 나 놈을 위한 것이다. 깃헙코스를 올리고 처음 구동할 때, 자신의 다른 VS Code의 환경과 연동이 되어 있다면 확장을 설치하는데 시간이 좀 걸린다. 한번만 참고 기다리면 된다. 
