...
데이터 앱을 만드는 최초의 단계는 데이터와 이를 다룰 도구(언어)의 연동이다. 별도로 DB를 만들지 않을 것이라면 데이터 소스는 아마도 구글 시트를 포함한 스프레드시트 형태일 것이다. 데이터 앱을 만들기로 했다면 가장 먼저 해야 하는 고민이 여기 있다. 파이썬과 구글 스프레드 시트는 어떻게 연동하면 될까? 이 포스트 역시 내용을 까먹을 미래의 나놈을 위한 것이다. 
