...
아래의 코드를 보자. 이 코드는 `:::{layout="[-5,10,-5]"} ... :::`가 감싸고 있으며, 그림의 크기는 `fig.set_size_inches(3, 3)`로 지정했다. `:::{layout...`에 해당하는 부분은 그림의 전체적인 크기를 조절하는 데 사용한다. R의 경우 이 지정만으로 그림의 비율에 맞게 줄어드는데, 파이썬의 경우에는 이렇게 작동하지 않았다. 아럐 사례를 보자. 
