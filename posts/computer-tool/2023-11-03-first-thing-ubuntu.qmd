...
보통 우분투를 설치하고 그 기기에 키보드와 마우스를 연결해서 쓰는 일은 드물지 않을까 싶다. WSL을 예로 들어 보자. 윈도와 나란히 설치된(윈도 내부에 있지만 사실상 독립된 형태로 존재한다) 리눅스 서버에 윈도에서 접속해 활용하는 구조가 WSL이다. 이렇게 다른 OS(Windows, Macos)에서 우분투 서버에 접속해서 쓸 때는 터미널 앱이 중요하다. 윈도에서는 [윈도 터미널](https://learn.microsoft.com/ko-kr/windows/terminal/install)이 Macos에서는 [iTerm2](https://iterm2.com/)이 좋다. 
...
기본 리포지토리 주소를 바꾸도록 하자. 만일 WSL을 통해 설치했다면 우분투 메인 리포(`http://archive.ubuntu.com/ubuntu/`)로 설정되어 있을 것이고, iso 이미지로 설치했다면, `kr`이 앞에 붙어 있다. apt 패키지 업데이트를 위한 주소는 `/etc/apt/sources.list` 파일을 직접 편집하면 된다. 에디터는 취향껏 쓰면 된다. 편리한 nano로 예를 들자. 
