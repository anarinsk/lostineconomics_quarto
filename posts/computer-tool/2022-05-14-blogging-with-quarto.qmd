...
author: "JS HUHH"
date: "05/14/2022"
image: "https://blogsspreadspot.com/wp-content/uploads/2021/11/blog.jpg"
...
상용 블로그를 쓰긴 권한다. 블로깅 툴은 관리가 편한 게 제일 좋다. 다만 사용 툴은 몇 가지 제약이 있다. 

-  $\LaTeX$ 수식의 제너레이션이 쉽지 않다. 
- 생각보다 깔끔한 툴이 별로 없다. 
- 왠지 모를 종속된 느낌적 느낌 
...
스태틱 웹을 이용할 수 있게 해주는 도구는 대체로 다른 도구에 종속되어 있다. 이 블로그의 최초 버전은 RStudio에서 제공하는 Blogdown이라는 도구를 활용했다. 작동하는 방식은 여느 스태틱 웹을 생성하는 툴과 비슷하다. 사용하기에 부족함이 없지만, R에 특화된 도구에 종속되어 있다는 것이 마음에 걸렸다. 

두번째 버전에서는 '꼼수'를 써 봤다. 깃헙의 각 리포지토리(리포)별로 html 호스팅이 가능하니까 포스팅별로 특화된 포스팅 페이지를 만들고 해당 페이지의 링크를 지닌 페이지를 블로그의 홈으로 만들었다. 포스팅이 별도의 깃헙 리포로 된 구조 때문에 글이 늘어가면 관리가 쉽지 않게 된다. 

세번째 버전은 fastpages를 활용했다. fastpages는 깃헙 액션스의 빌드 기능을 활용한다. 사용자는 `.md`로 콘텐츠를 전달하고 나머지 빌드는 모두 깃헙 위에서 이루어진다. 별도의 로컬 프로세스가 없다는 점에서 편리한 접근이지만, 디자인 활용이 제한적이고 프로젝트의 지원이 시원치 않아 보였다. 자세한 내용은 [여기](https://anarinsk.github.io/lostineconomics_quarto/posts/computer-tool/2020-03-07-blogging-with-fastpages.html)를 참고하자. 

## Quarto 

프로젝트의 홈에 따르면 Quarto는 과학적 문서 작성을 위한 독립된 도구다. 앞서 소개한 BlogDown과 같은 툴이 특정 도구에 의존한다면 이 녀석은 Jupyter, VS Code, RStudio 등 여느 에디터와 함께 쓸 수 있다. Quarto 프로젝트는 아직 초기지만 상당한 기대와 지지를 받고 있다. 이 프로젝트의 장점은 아래와 같다. 

-  마크 다운 기반의 문서 작성, 스태틱 웹 도구 그리고 계산 기반 문서 작성 도구의 기능을 부족하지 않게 두루 갖추고 있다. 
- 각종 기능에 관한 문서화가 체계적으로 잘 되어 있다. 
- CLI 기반이라서 도구 의존성이 낮다.  

프로젝트가 초기여서 안정성이 떨어지는 경우가 있고, 구글링을 통해 필요한 정보를 충분히 얻을 수 없는 단점이 있다. 시간이 지나면 해결될 것으로 생각한다.  

# Application 

[Quarto](https://quarto.org/)를 이용해 기존 fastpages 기반 블로그를 대체해보자. 기존 블로그 포스팅은 어떻게 가져올 수 있을까? 포스팅의 양이 많지 않을 경우 반복 작업을 해도 되고, 필요하면 간단한 스크립트를 통해서 일괄해서 바꿀 수 있다.  
...
먼저 전체적인 블로그의 구조를 생성해야 한다. 

```bash
? quarto create-project YOUR-BLOG-DIR --type website:blog
```

`YOUR-BLOG-DIR` 은 앞서 지정한 깃허브 리포의 디렉토리에 해당한다. 나머지는 그대로 쓰면 된다. 이 명령을 실행하면 해당 디렉토리에 블로그를 서비스하기 위해 필요한 파일들이 생성된다. 
...
`render` 명령어는 페이지를 렌더링하고 `preview`는 페이지를 미리 보여준다. VS Code의 Quarto extension을 쓰면 해당 과정을 편리하게 진행할 수 있다. 
...
`_quarto.yml`의 설정으로 별도의 `.css` 파일을 설정할 수 있다. 취향에 따라서 몇 가지 커스터마이즈가 도움이 될 수 있다. 
...
`posts/` 내 별도의 디렉토리를 만들어 포스팅을 구별할 수도 있다. 다만, 포스팅 `.qmd`을 작성할 때 별도의 `.html`을 삽입했다면, 해당 `.html`을 각 디렉토리마다 넣어줘야 한다. 