...
현재 깃헙에서 터미널에서 id/password를 통한 로그인을 막고 있다. 저장소 주소를 origin에 박고 id/password로 로그인 힐 수 없다. 위 명령어를 실행하면 조건에 따라서 로그인 절차를 상세하게 안내해준다. 그대로 따라서 로그인 하면 큰 불편함을 느끼지 않을 것이다. 
